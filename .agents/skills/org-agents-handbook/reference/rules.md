...
_Redirect shim_ (smaller, but rule body needs an active Read by the agent):
...
_Inline shim_ (heavier, but Cursor sees content immediately on activation; must be regenerated when the canonical file changes — a tiny script that copies `.agents/rules/*.md` → `.cursor/rules/*.mdc` with prepended frontmatter is the standard pattern):
