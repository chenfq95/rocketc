...
- 💡 **Sub-agent routing (`agent:` frontmatter)** is the one Commands-only capability that has no direct Skills equivalent today — if a workflow _must_ run inside a specific sub-agent's isolated context, keep it as a Command until opencode adds the equivalent field to skills, or have a thin `.opencode/commands/` wrapper that delegates to a Skill (see §5.4)
