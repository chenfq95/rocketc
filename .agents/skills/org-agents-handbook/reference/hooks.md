...
```json
{
  "preToolUse": [{ "matcher": "terminal", "command": ".cursor/hooks/lint-cmd.sh" }],
  "postToolUse": [{ "matcher": "editFile", "command": ".cursor/hooks/format.sh" }]
}
```
...
```json
{
  "hooks": {
    "SessionStart": [
      {
        ...
        "hooks": [
          { "type": "command", "command": "$HOME/.codex/hooks/load-context.sh", "timeout": 15 }
        ]
      }
    ],
    "PreToolUse": [
      {
        ...
        "hooks": [
          { "type": "command", "command": "$HOME/.codex/hooks/block-dangerous.sh", "timeout": 5 }
        ]
      }
    ],
    "PostToolUse": [
      {
        "matcher": "apply_patch",
        "hooks": [
          { "type": "command", "command": "$HOME/.codex/hooks/format-edited.sh", "timeout": 30 }
        ]
      }
    ],
    "UserPromptSubmit": [
    ...
    "Stop": [
      {
        "hooks": [
          { "type": "command", "command": "$HOME/.codex/hooks/session-summary.sh", "timeout": 30 }
        ]
      }
    ]
  }
...
```json
{
  ...
  "systemMessage": "Use rm -rf ./build instead."
}
```
...
3. **Never `exit 2` with a hostile message**. The stderr text is fed back to the model as guidance — write it like an error message the agent should learn from: _"Blocked: file X is read-only per `.agents/rules/protected-paths.md`. Use Y instead."_
