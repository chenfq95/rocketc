...
import { ChannelServer, type ServerChannel } from './channel.server';
import {
  type Protocol,
  type Subscribable,
...
export class IPCServer<TContext = string> {
  ...
  constructor(
    onClientConnect: Subscribable<{
      protocol: Protocol;
      onClientDisconnect: Subscribable<void>;
    }>,
  ) {
    this.subscription = fromSubscribable(onClientConnect).subscribe(
      async ({ protocol, onClientDisconnect }) => {
  ...
  getChannel(ctx: TContext, channel: string): IPCServerChannel {
    ...
    const channelClients = connections.map((connection) =>
      connection.channelClient.getChannel(channel),
    );
    return {
      invoke<TArgs extends any[] = any[], TReturn extends any[] = any>(
        command: string,
        ...args: TArgs
      ) {
        return Promise.all(
          channelClients.map((client) =>
            client.invoke<TArgs, ItemType<TReturn>>(command, ...args),
          ),
        );
      },
      event<TData>(event: string) {
        const observables = channelClients.map((client) =>
          fromSubscribable(client.event<TData>(event)),
        );
