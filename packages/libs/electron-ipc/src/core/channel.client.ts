import {
  ...
  type EventTriggerMessage,
  type InvokeCancelMessage,
  type InvokeFailMessage,
  type InvokeMessage,
  type InvokeSuccessMessage,
  type IPCMessage,
  MessageType,
  type Protocol,
  type RequestID,
  RequestType,
  type ClientSubscribable,
  type SubscribeEventMessage,
  type UnsubscribeEventMessage,
} from './common';
import { type CancellablePromise, Cancellation } from '../util/cancellation';
import { CustomizePromise } from '../util/customize-promise';
import { Observable, share, Subscription } from 'rxjs';

export interface ClientChannel {
  invoke<TArgs extends any[] = any[], TReturn = any>(
    command: string,
    ...args: TArgs
...
export class ChannelClient<TContext = string> {
  ...
  private requestId = 0;
  private readonly activeRequest: Map<
    RequestID,
    ActiveInvokeRequest | ActiveEventRequest
  > = new Map();
  ...
  constructor(
    ...
    this.subscription.add(
      this.protocol.onMessage.subscribe(this.handleIncomingMessage.bind(this)),
    );
  }
  ...
  private handleInvokeReturned(
    message: InvokeSuccessMessage | InvokeFailMessage | InvokeCancelMessage,
  ) {
    const { id, type } = message;
    const request = this.activeRequest.get(id)!;
    if (request.type !== RequestType.INVOKE) {
      throw new Error(
        'invoke request type not match, expect is invoke, actual is event',
      );
    }
  ...
  private handleEventTrigger(message: EventTriggerMessage) {
    ...
    if (request.type !== RequestType.EVENT) {
      throw new Error(
        'invoke request type not match, expect is event, actual is invoke',
      );
    }
  ...
  private invokeRemote<TArgs extends any[] = any[], TReturn = any>(
    ...
    const cancelablePromise = Cancellation.fromPromise(
      customizePromise.promise,
      (error) => {
  ...
  private subscribeRemote<TData = any>(channel: string, event: string) {
    for (const item of this.activeRequest.values()) {
      if (
        item.type === RequestType.EVENT &&
        item.request.channel === channel &&
        item.request.event === event
      ) {
