...
import { type Cancellable, Cancellation } from '../util/cancellation';
...
export class ChannelServer<TContext = string> {
  ...
  private readonly activeRequest = new Map<
    RequestID,
  ...
  registerChannel(
    channelName: string,
    serverChannel: ServerChannel<TContext>,
  ): void {
  ...
  private async handleInvoke(message: InvokeMessage): Promise<void> {
    ...
    try {
      ...
      const execution = Cancellation.fromPromise(
        Promise.resolve(serverChannel.invoke(this.ctx!, command, ...args)),
      );
  ...
  private async handleInvokeCancel(message: InvokeCancelMessage) {
    ...
    if (activeRequest.type === RequestType.EVENT) {
      throw new Error(
        `could not cancel execution(${id}) which is a subscription.`,
      );
    }
    this.activeRequest.delete(id);
  ...
  private async handleSubscribeEvent(message: SubscribeEventMessage) {
    ...
    const subscription = serverChannel
  ...
  private async handleUnsubscribeEvent(message: UnsubscribeEventMessage) {
    ...
    if (!activeRequest) {
      ...
      throw new Error(
        `could not unsubscribe subscription(${id}) which is not exist.`,
      );
    }
    if (activeRequest.type === RequestType.INVOKE) {
      throw new Error(
        `could not unsubscribe subscription(${id}) which is a execution.`,
      );
    }
    this.activeRequest.delete(id);
