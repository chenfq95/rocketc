...
describe('connection', () => {
  it('should create a connection between client and server', async () => {
    const { clientProtocol, serverProtocol, onDidClientConnect } =
      createProtocol();
...
`);
  ...
  it('ipcClient should send disconnect message when destroyed', async () => {
    const { clientProtocol, onDidClientConnect, eventEmitter } =
      createProtocol();
...
describe('invoke', () => {
  it('client should receive server returned value while invoke channel', async () => {
    ...
    const result2 = ipcClient
      .getChannel('test')
      .invoke('test_command', 1, 2, 3);
  ...
  it('client should receive unknown channel error while channel not found', async () => {
    ...
    const execution = ipcClient
      .getChannel('test')
      .invoke('test_command', 1, 2, 3);
    await expect(execution).rejects.toThrowErrorMatchingInlineSnapshot(
      `"unknown channel: test"`,
    );
  });
  ...
  it('server should not return cancel error while client cancel request manual.', async () => {
    ...
    expect(serverProtocolSpy).toHaveBeenCalledTimes(2);
  });
  ...
  it('server should return array result while invoke client', async () => {
    ...
    await delay(0);
    const execution = ipcServer
      .getChannel('client', 'test')
      .invoke('test_command', 1, 2, 3);
...
describe('subscribe', () => {
  it('subscribe message should not send before been subscribed.', async () => {
    ...
    await delay(0);
    expect(serverEvent).toHaveBeenCalledTimes(1);
  });
  ...
  it('server should receive event from client after subscribe.', async () => {
    ...
    await delay(0);
    expect(clientEvent).toHaveBeenCalledTimes(1);
    const eventData = { code: 0 };
    subject.next(eventData);
    await delay(0);
    expect(cb).toHaveBeenCalledTimes(1);
